...
const validationSchema = Yup.object({
  ...
  status: Yup.string()
    .oneOf(["complete", "pending"], "Invalid status")
...
const EditTaskForm = ({ taskId }) => {
  ...
  const getDataById = async (id) => {
    try {
      const res = await fetch(`${apiUrl}/${taskId}?userId=${id}`, {
        method: "GET",
      });
      if (res?.ok) {
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      // Once the request is done, re-enable the submit button
      setSubmitting(false);
    }
  };
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center mb-2">
                  <Field
                    ...
                    className="mr-2"
                  />
                  <label htmlFor="complete" className="text-gray-700">
                    Complete
                  </label>
                </div>
                <div className="flex items-center mb-2">
